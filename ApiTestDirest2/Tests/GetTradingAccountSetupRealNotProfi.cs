...
namespace ApiTestDirest2
{
    public partial class ApiTestDirect2
    {
        ...
        public void GetTradingAccountSetupRealNotProfi(string authority, string country, string platform)
        {
            #region Test Data
            Dictionary<string, string> expList = new Dictionary<string, string>();
            expList.Add("mena_AE_mt4m", System.IO.File.ReadAllText(jsonResponses + "GetTradingAccountSetupRealNotProfi.authority=mena.country=AE.platform=mt4m.json", Encoding.Default));
            expList.Add("mena_AE_xtrader", System.IO.File.ReadAllText(jsonResponses + "GetTradingAccountSetupRealNotProfi.authority=mena.country=AE.platform=xtrader.json", Encoding.Default));

            JObject expectedResponse = JObject.Parse(expList[authority + "_" + country + "_" + platform]);
            #endregion

            JObject actualResponse;
            ApiHelper.GetTradingAccountSetup(BaseUrlApi, "true", "false", authority, country, platform, out actualResponse);
